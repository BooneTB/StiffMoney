...
contract Stiff is BEP20Token,PriceConsumerV3{
    address public fiatContractAddress;

    constructor() PriceConsumerV3() public {
    }
    
  
    
    function mintToken(address _addres,uint256 _amount) public onlyOwner() {
        _mint(_addres,_amount*10**(18));
    }
    
   
    function buy() payable  public{
        require(msg.value > 0 ether,"DEED: No value transfered");
        uint256 weiUSD = (uint256)(getLatestPrice());
        require(weiUSD != 0, "POH: No exchange value returned. Try again");
        //calculating amount of DEED Token to be minted.
        uint256 unitPrice = msg.value.div(weiUSD);
        uint256 amountOfPOHTokens = (10**uint256(18)) * unitPrice; //1 DEED token * USD amount of Value
